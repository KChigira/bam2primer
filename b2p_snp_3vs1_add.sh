...
Usage: $(basename "$0") -R referance_genome.fasta [OPTION]... Input_VCF.vcf
  ...
  -c chromosome     default:chr01,chr02
  -a physical position start     default:0,10000000
  -e physical position end       default:5000000,15000000
EOM
  exit 2
}
...
POS_CHR="chr01,chr02"
POS_S="0,10000000"
POS_E="5000000,15000000"

while getopts ":R:n:d:p:b:s:m:t:x:y:c:a:e:h" optKey; do
  case "$optKey" in
    R) REF=${OPTARG};;
    n) NAME=${OPTARG};;
    d) MIN_DEP=${OPTARG};;
...
perl perl/select_snp_3vs1_add.pl ${VCF} \
                        ${CURRENT}/${NAME}_selected.vcf \
                        ${MIN_DEP} ${MAX_DEP} ${BETWEEN} ${COL_X} ${COL_Y} ${POS_CHR} ${POS_S} ${POS_E}
if test $? -ne 0 ; then
