...
while($line = <$fh_in>){
  ...
  my @geno_c = split(/:/, $tmp_2[11]);
  my @geno_d = split(/:/, $tmp_2[12]);
  for (my $i = 0; $i < $formatlen; $i++){
    if($geno_format[$i] eq "GT"){
      @genotype = ($geno_a[$i], $geno_b[$i], $geno_c[$i], $geno_d[$i]);
    }elsif($geno_format[$i] eq "DP"){
      @depth = ($geno_a[$i], $geno_b[$i], $geno_c[$i], $geno_d[$i]);
    }
  }

  #when genotype is different between samples
  #and depth is within the range, write output vcf
  if(($genotype[0] eq "0/0" && $genotype[1] eq "0/0" && $genotype[2] eq "0/0" && $genotype[3] eq "1/1") ||
     ($genotype[0] eq "1/1" && $genotype[1] eq "1/1" && $genotype[2] eq "1/1" && $genotype[3] eq "0/0")){
    if($depth[0] >= $min_dp && $depth[0] <= $max_dp &&
       $depth[1] >= $min_dp && $depth[1] <= $max_dp &&
       $depth[2] >= $min_dp && $depth[2] <= $max_dp &&
       $depth[3] >= $min_dp && $depth[3] <= $max_dp){
