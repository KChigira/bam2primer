...
my ($INVCF, $FAI, $TARGET_NUM, $INTERVAL);
my $line;
my $before;
my $column;
...
print $fh_outtxt "Chr\tPos\tID\tLeft\tRight\tL_TM\tR_TM\tProduct_Size\t".$tmp2[9]."\t".$tmp2[10]."\t".$tmp2[11]."\t".$tmp2[12]."\n";
for(my $i = 0; $i < $num; $i++){
  ...
  if(substr($filtered[$i][12], 0, 3) eq "0/0"){
    push(@o_line, "Ref");
  }else{
    push(@o_line, "Alt");
  }
