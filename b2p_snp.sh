...
#Make VCF from BAM
if [ -d ${CURRENT}/${NAME}_vcf ]; then
  echo "The result of same sample name exists."
  usage
fi
mkdir ${CURRENT}/${NAME}_vcf
bash mkvcf.sh ${REF} \
              ${BAM_1} \
              ${CURRENT}/${NAME}_vcf \
              ${NAME_BAM_1} \
...
#
bcftools merge -o ${CURRENT}/${NAME}_vcf/${NAME}_merged_filtered_variants.vcf.gz \
               -O z \
               ${CURRENT}/${NAME}_vcf/${NAME_BAM_1}_filtered_variants.vcf.gz \
               ${CURRENT}/${NAME}_vcf/${NAME_BAM_2}_filtered_variants.vcf.gz
bcftools index ${CURRENT}/${NAME}_vcf/${NAME}_merged_filtered_variants.vcf.gz
#force calling the position where alleles exists.
#To make genotype not "./." but "0/0"
bash mkvcf_2nd.sh ${REF} \
                  ${BAM_1} \
                  ${CURRENT}/${NAME}_vcf \
                  ${NAME_BAM_1} \
                  ${CURRENT}/${NAME}_vcf/${NAME}_merged_filtered_variants.vcf.gz \
                  &
bash mkvcf_2nd.sh ${REF} \
                  ${BAM_2} \
                  ${CURRENT}/${NAME}_vcf \
...
bcftools merge -o ${CURRENT}/${NAME}_merged_final_variants.vcf \
               -O v \
               ${CURRENT}/${NAME}_vcf/${NAME_BAM_1}_filtered_variants_recal.vcf.gz \
               ${CURRENT}/${NAME}_vcf/${NAME_BAM_2}_filtered_variants_recal.vcf.gz
